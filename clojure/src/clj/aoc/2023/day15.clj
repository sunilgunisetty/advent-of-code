...
(defn parse-sequence
  [data]
  (let [[_ data op val] (re-find #"([a-z].*)([=-]{1})([\d]?.*)" data)]
    [data op val]))
...
(defn fill-boxes
  ...
  (reduce
   (fn [acc operation]
     (let [[data op val] (parse-sequence operation)
           k             (hash-algo (map int data))]
       (if (= op "-")
         (update acc k (fn [old] (filterv (fn [[v _]] (not= v data)) old)))
         (if (seq (filter (fn [[d _]] (= d data)) (get acc k)))
           (update acc k (fn [old] (mapv (fn [[d v]] (if (= d data) [d val] [d v])) old)))
           (update acc k (fnil conj []) [data val])))))
   {}
   operations))
...
(defn focusing-power
  ...
  (reduce-kv
   (fn [acc k data]
     (if (seq data)
       (+ acc (->> data
                   (map-indexed (fn [idx [_ v]] (* (inc idx) (Integer/parseInt v))))
                   (map (fn [v] (* v (inc k))))
                   (apply +)))
       acc))
   0 boxes))
...
(defn day15-part2
  [input]
  (-> input (string/split #",") fill-boxes focusing-power))
