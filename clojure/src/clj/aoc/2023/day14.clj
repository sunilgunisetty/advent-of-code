...
(def input (->> "../input/2023/day14.txt" slurp))
(def sample-input (->> "../input/2023/day14-sample.txt" slurp))
...
(defn tilt
  ...
  (mapv
   (fn [val]
     (string/join "#" (map compact (string/split val #"#" -1))))
   input))
...
(defn tilt-east
  [input]
  (->> input (mapv #(->> % reverse (apply str))) tilt (mapv #(->> % reverse (apply str)))))

(defn tilt-south
  [input]
  (->> input transpose (mapv #(->> % reverse (apply str))) tilt transpose reverse))

(def memo-tilt-north (memoize tilt-north))
(def memo-tilt-west (memoize tilt-west))
...
(def memo-spin-cycle (memoize spin-cycle))
